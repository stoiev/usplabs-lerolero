...
def generate(number_of_paragraphs=1, min_sentences_per_paragraph=1,
             max_sentences_per_paragraph=1):

    lerolero = []

    for p in range(0, number_of_paragraphs):
        paragraph = ""
        number_of_sentences = random.randrange(min_sentences_per_paragraph,
                                               max_sentences_per_paragraph + 1)
        for s in range(0, number_of_sentences):
            paragraph += random.choice(t0) + \
                        random.choice(t1) + \
                        random.choice(t2) + \
                        random.choice(t3) + " "
        lerolero.append(paragraph)
